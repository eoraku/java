...
public class Sea implements ActionModel {
	...
	public void execute(HttpServlet servlet, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		query += " WHERE WN_WHEN = '0' AND RC_CATEGORY = '3'";
		query += " ORDER BY RC_ORDER";
		/*
		SELECT WN_REGION, RC_NAME, WN_RAINF, SUBSTRING(WN_FORECAST, 4) WN_FORECAST, WN_WINDA, WN_WTRAND, WN_WINDB,
				CONCAT(ROUND(WN_WSPEEDA), '~', ROUND(WN_WSPEEDB)) WN_WSPEED, CONCAT(WN_WHEIGHTA, '~', WN_WHEIGHTB) WN_WHEIGHT, RC_X, RC_Y, RC_ORDER
			FROM WEATHER_SIMPLE LEFT JOIN REGION_CODE ON WN_REGION = RC_CODE
			WHERE WN_WHEN = '0' AND RC_CATEGORY = '3'
			ORDER BY RC_ORDER
		 */
