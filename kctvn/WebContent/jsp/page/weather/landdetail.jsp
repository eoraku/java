...
$D.create("img").setPath("${imgRoot}/weather/04-1-1_bg.png");
...
<script type='text/javascript'>
	...
	function setBg(){
		$D.create("img").setPos(0, 0).setPath("${imgRoot}/weather/04-1-1_bg.png").draw();
	...
	function setTableData(){
		...
		//var rowDescr = [ "날짜", "시간", "기온(<img src='${imgRoot}/weather/temp.png'/>)", "날씨", "풍속<br>(m/s)", "" ];
	...
	function setDays(){
		...
		if(data[0]["W3_TIME"].substring(8, 10) == "21"){ // 첫번째 시간이 21일때
			day[0] ++;
		}else{
			for ( var i = 0; i < data.length; i++) {
