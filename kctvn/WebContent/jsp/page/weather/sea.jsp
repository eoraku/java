...
if(obj.length != 0 && obj != ""){
	$D.create("img").setPath("${imgRoot}/weather/04-2_bg.png");
...
function drawBg(){
	$D.create("img").setPos(0,0).setPath("${imgRoot}/weather/04-2_bg.png").draw();//
	//$D.create("img").setPos(44,25).setPath("${imgRoot}/common/st_02.png").draw();
	//$D.create("font").setPos(78,30).setFontSize(18).setColor("#0e3c5d").setText("날씨 | 해상 예보").draw();
...
function setArea(){	
	...
	for (var i = 0; i < obj.length; i++) {
		...
		$D.create("img").setPos(13,8).setSize(39,34).setPath("${imgRoot}/common/w_icon_m/" + obj[i].WN_RAINF + obj[i].WN_FORECAST+".png").draw(on);
		$D.create("font").setId("areaname").setAlign("center").setPos(57,15).setFontSize(16).setColor("#fff566").setText(obj[i].RC_NAME).draw(on);
		...
		var off = $D.create("div").setPos(obj[i].RC_X,obj[i].RC_Y).setSize(620, 280).setDepth(1);		
		$D.create("img").setPos(0,0).setPath("${imgRoot}/weather/04-2_map_nrml.png").draw(off);	
		$D.create("img").setPos(13,8).setSize(39,34).setPath("${imgRoot}/common/w_icon_m/" + obj[i].WN_RAINF + obj[i].WN_FORECAST+".png").draw(off);
		$D.create("font").setId("areaname").setAlign("center").setPos(57,15).setFontSize(16).setColor("#205879").setText(obj[i].RC_NAME).draw(off);
		...
		$D.create("font").setId("areainfo3").setAlign("left").setPos(57,79).setSize(120,22).setFontSize(15).setColor("#464646").setText("파고 " + obj[i].WN_WHEIGHT + "m").draw(off); //obj[i].WN_WHEIGHT
...
$K.enter(function(){
	...
	param.list = obj;
	$H.foward("${ctxRoot}/seadetail.do?wn_region=" + obj[menu.getIndex()].WN_REGION, param);
});
